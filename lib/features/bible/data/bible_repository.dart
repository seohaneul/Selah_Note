...
class BibleRepository {
  ...
  static final Map<String, int> chapterMaxVerses = {}; // key: "창1", value: max verse

  static final Map<String, String> bookFullNames = {
  ...
  static String getFullName(String abbrev) => bookFullNames[abbrev] ?? abbrev;
    ...
    try {
      ...
      data.forEach((key, value) {
        ...
        if (match != null) {
          ...
          String chapKey = "$bookName$chapter";
          if ((chapterMaxVerses[chapKey] ?? 0) < verse) {
            chapterMaxVerses[chapKey] = verse;
          }
        }
      });
  ...
  static List<Verse> getChapterVerses(String bookName, int chapter) {
    ...
    for (var entry in matchingEntries) {
      ...
      if (parts.length == 2) {
        ...
        if (verseNum > 0) {
          verses.add(Verse(
            bookName: getFullName(bookName), // 풀네임 적용
            chapter: chapter,
            verse: verseNum,
